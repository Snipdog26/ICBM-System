...
namespace IngameScript
{
    partial class Program : MyGridProgram
    {
        ...
        double
            ...
            _maxAimDispersion = 0,
            _topDownAttackHeight = 20500,
            _heatActivationDistance = 10,
...
"We've been trying to reach you about your car's extended warranty",
        ...
        readonly GPSHoming _gpsHoming;

        enum GuidanceMode : int { BeamRiding = 1, SemiActive = 2, GPS = 3, Active = 4, Homing = SemiActive | Active };
        ...
        Program()
        {
            ...
            _scheduler.AddScheduledAction(GuidanceNavAndControl, UPDATES_PER_SECOND); // Gets target data too
        ...
        void LoadIniConfig()
        {
            ...
            _topDownAttackHeight = 50000;//_myIni.Get(INI_SECTION_HOMING, INI_TOPDOWN_ATTACK_HEIGHT).ToDouble(_topDownAttackHeight);
        ...
        void HandleBroadcastListeners()
        {
            ...
            /*
                GPS Guidance 
                Needed Data:
                    Item1 = Position ( Used for Sub-Orbital trajectory)
                    Item2 = Keycode ( missile indexing)
        ...
        void RegisterBroadcastListeners()
        {
            ...
            _broadcastListenerGPS = IGC.RegisterBroadcastListener(IGC_TAG_GPS);
            _broadcastListenerGPS.SetMessageCallback(IGC_TAG_GPS);
            ...
            out bool shouldProximityScan)
        {
            ...
            else if (guidanceMode == GuidanceMode.Homing)
            {
        ...
        Vector3D GPSGuidance(Vector3D missilePos,
            Vector3D missileVel,
            Vector3D gravityVec,
            double missileAcceleration,
            out Vector3D adjustedTargetPos)
        ...
        void CheckProximity()
        {
            ...
            Vector3D closingVelocity = (_guidanceMode == GuidanceMode.BeamRiding || _guidanceMode == GuidanceMode.GPS)
                 ? missileVelocity
                 : missileVelocity - _targetVel;
            // If Beamriding or GPS -> then closing velocity is missile velocity. Else solve for it using target and missile vel.
            ...
            double closingSpeed;
            // If we have no cameras or sensors for detonation, use some approximations/ or if in HEAT mode.
            if ((_cameras.Count == 0 && _sensors.Count == 0) || _heatModeActive)
            {
                ...
                else
                {
                    ...
                    if (_heatModeActive)
                    {
                        if ((distanceToTarget + _heatActivationDistance) < adjustedDetonationRange + closingSpeed * SECONDS_PER_UPDATE)
                        {
                            Detonate((distanceToTarget + _heatActivationDistance - adjustedDetonationRange) / closingSpeed);
                            return;
                        }
                    }
                    else
